...
export function Collections() {
    ...
    const outfits: Outfit[] = [
        ...
        { id: 25, name: "Celestine's #1 Outfit", image: "src/assets/clothe25.png", description: "Open-back halter dress with a bright yellow, green, and blue floral print", size: "XS", category: "Dress" }, { id: 26, name: "Celestine's #2 Outfit", image: "src/assets/clothe26.png", description: "Strapless halter-style maxi dress with a vibrant purple and pink paisley print", size: "0", category: "Dress" },
    ...
    return (
        <div className="min-h-screen bg-[#FFF0C8]">
            {/* Header */}
            <div className="bg-[#5B1B3A] sticky top-0 z-40 shadow-lg">
                <div className="container mx-auto px-4 py-6">
                    <div className="flex items-center justify-between mb-6">
                        <button
                            onClick={() => window.history.back()}
                            className="text-white hover:text-[#AD7301] transition-all duration-300 transform hover:scale-110 flex items-center space-x-2"
                        >
                            <ArrowLeft size={24} />
                    ...
                    <div className="relative max-w-2xl mx-auto">
                        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#5B1B3A]" size={20} />
                        <input
                            type="text"
                            placeholder="Search outfits by name or description..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-10 pr-4 py-3 rounded-full border-2 border-[#AD7301] focus:outline-none focus:border-[#891B81] text-[#5B1B3A] placeholder-[#5B1B3A]/60"
                        />
                    </div>
                </div>
            </div>
            ...
            {/* Filter Panel */}
            {showFilters && (
                <div className="bg-[#891B81] text-white p-6 shadow-lg">
                    <div className="container mx-auto">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-xl font-semibold">Filter Outfits</h3>
                            <div className="flex space-x-3">
                                {hasActiveFilters && (
                                    <button
                                        onClick={clearFilters}
                                        className="bg-[#940000] text-white px-4 py-2 rounded-full hover:bg-[#AD7301] transition-all font-medium"
                                    >
                                        Clear All
                                    </button>
                                )}
                                <button
                                    ...
                                    className="bg-white text-[#5B1B3A] px-4 py-2 rounded-full hover:bg-[#FFF0C8] transition-all font-medium"
                                >
                                    Close
                                </button>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full bg-white text-[#5B1B3A] rounded-xl px-4 py-3 border-2 border-[#AD7301] focus:outline-none focus:border-[#4E4F06]"
                                >
            ...
            <div className="container mx-auto px-4 py-6">
                <div className="flex justify-between items-center mb-6">
                    <p className="text-[#5B1B3A] text-lg">
                        Showing {filteredOutfits.length} of {outfits.length} outfits
                        {hasActiveFilters && (
                            <span className="text-[#891B81] font-medium">
                {' '}(filtered)
              </span>
                        )}
                    </p>
                    {hasActiveFilters && (
                        <button
                            onClick={clearFilters}
                            className="text-[#940000] hover:text-[#891B81] font-medium transition-all"
                        >
                            Clear filters
                        </button>
                    )}
                ...
                {filteredOutfits.length === 0 ? (
                    <div className="text-center py-16">
                        <div className="text-[#5B1B3A] text-xl mb-4">No outfits found</div>
                        <button
                            onClick={clearFilters}
                            className="bg-[#891B81] text-white px-6 py-3 rounded-full hover:bg-[#940000] transition-all font-medium"
                        >
                            Clear all filters
                        </button>
                    </div>
                        ...
                        {filteredOutfits.map((outfit) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div className="absolute top-4 left-4 bg-[#5B1B3A] text-white px-3 py-1 rounded-full text-sm font-medium shadow-lg">
                                        {outfit.size}
                                    </div>
                                    <div className="absolute top-4 right-4 bg-[#AD7301] text-white px-3 py-1 rounded-full text-sm font-medium shadow-lg">
                                        {outfit.category}
                                    </div>
                                </div>
                                <div className="p-6">
                                    <h3 className="font-bold text-[#5B1B3A] text-lg mb-2 group-hover:text-[#891B81] transition-colors">
                                        {outfit.name}
                                    </h3>
                    ...
                    <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="relative">
                            ...
                            <button
                                ...
                                className="absolute top-4 right-4 bg-white rounded-full p-2 hover:bg-gray-100 transition-all transform hover:scale-110 shadow-lg"
                            >
                        ...
                        <div className="p-8">
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-8">
                                <div className="bg-[#FFF0C8] p-4 rounded-xl border-2 border-[#AD7301]">
...
function ReservationForm({ outfit, onClose }: { outfit: Outfit; onClose: () => void }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        ...
                        required
                        className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#891B81] focus:border-transparent text-[#5B1B3A]"
            ...
            <div className="bg-[#FFF0C8] p-6 rounded-xl border-2 border-[#AD7301]">
                <div className="flex items-start space-x-4">
                    <input
                        ...
                        onChange={handleChange}
                        required
                        className="mt-1 w-5 h-5 text-[#891B81] focus:ring-[#891B81] border-gray-300 rounded"
                    />
            ...
            <div className="flex space-x-4">
                <button
                    type="button"
                    onClick={onClose}
                    className="flex-1 bg-gray-300 text-[#5B1B3A] py-4 rounded-xl font-semibold hover:bg-gray-400 transition-all"
                >
                    Cancel
                </button>
                <button
                    ...
                    className="flex-1 bg-[#940000] text-white py-4 rounded-xl font-semibold hover:bg-[#891B81] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-xl"
                >
                    Submit Reservation
                </button>
            </div>
