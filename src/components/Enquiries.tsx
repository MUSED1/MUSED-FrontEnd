// components/Enquiries.tsx
//
// General "get in touch" contact form — where the footer's "Enquiries"
// link points. Distinct from /partner-with-us (brand-specific outreach);
// this is for anyone with a general question. Public, no account
// required — posts to POST /api/contact, which emails support@mused852.com.
...
export function Enquiries() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError('');
        try {
            const res = await fetch(`${API_CONFIG.baseURL}${API_CONFIG.endpoints.contact}`, {
                method: 'POST',
            ...
            <main className="min-h-screen bg-cream pt-28 pb-20 sm:pt-32">
                <div className="container mx-auto max-w-2xl px-4">
                    ...
                    <div className="mb-10 text-center">
                        <span className="text-xs uppercase tracking-label text-plum/40">Get in Touch</span>
                        ...
                        <p className="mt-4 font-sans italic text-plum/70">
                            Questions, feedback, or just want to say hi? Send us a message and we'll get back to you.
                        </p>
                    </div>
                    ...
                    {submitted ? (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5 rounded-3xl bg-white/60 backdrop-blur-md border border-white/60 p-6 sm:p-8 shadow-[0_2px_10px_rgba(93,27,58,0.06)]">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-plum/80 mb-2">Message</label>
                                <textarea
                                    value={message}
