...
// Suggested route: /forgot-password
                    ...
                    {!sent ? (
                        ...
                        <div className="text-center py-4">
                            ...
                            <p className="text-plum/70 text-sm font-inter leading-relaxed mb-6">
                                If <strong className="text-plum">{email}</strong> is registered, you'll receive a link in the next few minutes. Don't forget to check your spam folder.
                            </p>
                            <button
...
// Suggested route: /reset-password?token=<rawToken>
// ─────────────────────────────────────────────────────────────────────────────
        ...
        fetch(`${API_URL}/auth/validate-reset-token/${token}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    setError(data.message || 'The link is invalid or has expired.');
                }
            })
            .catch(() => setError('Could not verify the link. Please try again later.'))
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setError(data.message || 'An error occurred. Request a new link.');
            }
        } catch {
            setError('Could not connect to the server. Please try again later.');
        } finally {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-cream via-cream to-rose/30">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-plum/60 font-inter text-sm">Verifying link...</p>
                </div>
            </div>
        );
    }
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl p-8 md:p-10 border border-gold/20">
                    ...
                    {!tokenValid && !success && (
                        <div className="text-center py-4">
                            ...
                            <h3 className="font-amandine text-3xl text-burgundy mb-3">Invalid link</h3>
                    ...
                    {success && (
                        <div className="text-center py-4">
                            ...
                            <h3 className="font-amandine text-3xl text-burgundy mb-3">Password updated!</h3>
                            <p className="text-plum/70 text-sm font-inter">Redirecting to your profile...</p>
                        </div>
                    )}
                            ...
                            <form className="space-y-5" onSubmit={handleSubmit}>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        />
                                        {confirm && confirm !== password && (
                                            <p className="text-red-500 text-xs mt-1 font-inter">Passwords do not match</p>
                                        )}
                                    </div>
                                </div>
