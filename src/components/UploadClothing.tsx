...
export function ClothingUploadForm() {
    ...
    const handleClothingItemChange = (index: number, field: keyof ClothingItem, value: string | File | undefined) => {
    ...
    const handleImageUpload = (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            handleClothingItemChange(index, 'imageFile', file)

            // Also create preview for display
            const reader = new FileReader()
            reader.onload = (e) => {
                const base64Image = e.target?.result as string
                handleClothingItemChange(index, 'image', base64Image)
            }
            reader.readAsDataURL(file)
        }
    }

    // Function to upload images to the new image API
    const uploadImagesToAPI = async (files: File[]): Promise<string[]> => {
        ...
        try {
            const response = await fetch('https://mused-backend.onrender.com/api/images/upload-multiple', {
                method: 'POST',
                body: formData,
            });
            ...
            if (response.ok && result.success) {
                ...
                return result.data.map((img: CloudinaryImage) => img.cloudinaryUrl);
            } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const validItems = clothingItems.filter(item => item.imageFile && item.category && item.size)
        ...
        try {
            // Step 1: Upload images to Cloudinary via our API
            const imageFiles = validItems.map(item => item.imageFile as File);
            const cloudinaryUrls = await uploadImagesToAPI(imageFiles);

            // Step 2: Prepare clothing items with Cloudinary URLs
            const clothingItemsWithUrls = validItems.map((item, index) => ({
                image: cloudinaryUrls[index], // Use Cloudinary URL instead of base64
            ...
            setSubmitMessage({
                type: 'error',
                message: error instanceof Error ? error.message : 'Connection error. Please check your internet and try again.'
            })
        } finally {
            setIsSubmitting(false)
        }
    ...
    const removeImage = (index: number) => {
        handleClothingItemChange(index, 'image', '');
        handleClothingItemChange(index, 'imageFile', undefined);
    }
                    ...
                    {isSubmitting && (
                        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
                            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md mx-4 text-center">
                                ...
                                <p className="text-plum/80 mb-2">Processing images and saving your items...</p>
